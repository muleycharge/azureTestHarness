...
while($messageCount -ne 0 -and $retry -lt $maxRetry){
    ...
    # Get message counts
    $subShow = az servicebus topic subscription show --ids $configs.test1Subscription1ResourceId | convertfrom-json
    $messageCount = $subShow.countDetails.activeMessageCount
    $subShow = az servicebus topic subscription show --ids $configs.test1Subscription2ResourceId | convertfrom-json
    $messageCount += $subShow.countDetails.activeMessageCount


    # Wait for ServiceBusSender to put messages on the queue or to fail before exiting.
    if($messageCount -eq 0 -and $sendMessageJob.State -ne "Completed"){
